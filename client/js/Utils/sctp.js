  ...
  return b = {
    ...
    setHeader: function (cmd, flags, id) {
      ...
      if (id) {
        ...
        this.writeUint32(++command_counter);
      }
      this.writeUint32(s);
    },
...
SctpClient = function () {
  ...
  this.events = {};
  // Map<CommandId, {success, fail}>
  this.currentlyExecutingCommands = {};
};
...
SctpClient.prototype.connect = function (url, success) {
  ...
  this.socket.onmessage = this._handle_response_to_command.bind(this);
...
SctpClient.prototype.erase_element = function (addr) {
  ...
  return this.new_request(buffer, buffer.data, function (data) {
    return null;
  });
};
...
SctpClient.prototype.iterate_elements = function (iterator_type, args) {
  ...
  this.new_request(buffer, buffer.data, function (data) {
    var res = [];
...
SctpClient.prototype.event_emit = function () {
  ...
  this.new_request(buffer, buffer.data)
    .done(function (data) {
      var n = data.getResUint32(0);
